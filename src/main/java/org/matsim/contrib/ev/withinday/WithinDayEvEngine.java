...
public class WithinDayEvEngine implements MobsimEngine, ActivityStartEventHandler, ChargingStartEventHandler,
	...
	private ChargingProcess createChargingProcess(Id<Person> personId, double now, ChargingSlot slot,
		...
		if (process.vehicle == null) {
			...
			System.out.println("person: " + process.agent.getId());
			System.out.println("vehicle: " + vehicleId);
			System.out.println("mode: " + chargingMode);

			Vehicle baseVehicle = vehicles.getVehicles().get(vehicleId);
			System.out.println("base vehicle: " + baseVehicle);

			if (baseVehicle != null) {
				System.out.println("base vehicle type: " + baseVehicle.getType().getId());
				System.out.println("engine technology: " + VehicleUtils.getHbefaTechnology(baseVehicle.getType().getEngineInformation()));
			}
		}
