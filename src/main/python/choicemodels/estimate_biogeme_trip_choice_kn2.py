...
if __name__ == "__main__":
    ...
    parser.add_argument("--est-price-perception-car", help="Estimate price perception", action="store_true", default=1)
    parser.add_argument("--est-price-perception-pt", help="Estimate price perception", action="store_true", default=1)
    ...
    UTIL_MONEY = Beta('UTIL_MONEY', 1, 0.0, 1.5, ESTIMATE if args.est_util_money else FIXED)
    ...
    for i, mode in enumerate(ds.modes, 1):
        ...
        price += v[f"{mode}_daily_costs"] * v["dist_weight"] * (BETA_CAR_PRICE_PERCEPTION if mode == "car" else BETA_PT_PRICE_PERCEPTION)
